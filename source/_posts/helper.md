...
```
Hexo 默认按时间倒序排列文章，hexo-generator-index 就是这个排序功能的插件。要启用置顶，需要先把它换掉。
...
```
这个插件会替换原来的排序功能，在保留按时间排序的基础上，增加置顶能力。
...
npm list --depth=0
```
检查输出中是否已没有 hexo-generator-index，但有 hexo-generator-index-pin-top。

#### 使用方法
在需要置顶的文章的 Front-matter（--- 区域）中添加 top 字段即可。
E.g.
```yml
---
title: 这是一篇置顶文章
date: 2024-06-01 11:45:00
tags:
  - 教程
top: true    # 添加这一行，即可置顶
---
...
如果有多篇置顶文章，你可以给 top 设置一个数字，数字越大，文章排得越靠前
